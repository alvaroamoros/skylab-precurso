...
pasapalabraMain = (questionData, userData, historyData) => { 
    restartVars (questionData); 
    greateUser (); 
    indexLetters ();
    askQuestion (questionData);
    sumPoints (questionData);
    saveData (userData, historyData);
    sortingAlgorithm (historyData);
    printResults (historyData);

 }
// Global vars
const historyResults = [];

saveData = (userResults) => { 
    let user = {name: userResults.user, points: userResults.points};
    historyResults.push(user);  
}

sortingAlgorithm = (historyResults) => { 
        if (historyResults.length > 1) {
        for (let t = 0; t < historyResults.length; t++) {
            for (let i = 0, j = 1;  j < historyResults.length; i++, j++) {
                let temp;
                if (historyResults[j].points > historyResults[i].points) {
                    temp = historyResults[i];
                    historyResults[i] = historyResults[j];
                    historyResults[j] = temp;
...
restartVars = (questionData) => {
    index = []; 
    userResults.user = "Guest"; userResults.points = 0;
    for (let i = 0; i < questionData.length; i ++) {
        questionData[i].status = 0;
    }
}


// Preguntas pasapalabra
// Obtenidas del repositorio público de Sara Mansori
const questions = [
    {
        letter: "A",
...
pasapalabraMain(questions, userResults, historyResults);
